...
class OpenVLAPolicy:
    ...
    def get_action(self, x: dict, deterministic) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        values, action, logprobs = vla_model.predict_action_batch(
            **features,
    ...
    def evaluate_actions(self, x: dict, action: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        vla_model = self.vla.module if hasattr(self.vla, "module") else self.vla
        logprobs, entropy, values, _ = vla_model.evaluate_action(
            **features,
            unnorm_key=self.args.vla_unnorm_key
        )
    ...
    def save(self, path: Path, epoch: int = None, best_epoch: int = -1, best_metric_value: float = -1.0):
...
class OpenVLAPPO:
    def __init__(self, all_args, policy: OpenVLAPolicy):
        ...
        self.rank = self.policy.device_id
