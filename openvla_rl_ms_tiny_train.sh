...
VLA_PATH="../RL4VLA/weights/openvla-7b-rlvla-warmup"
...
if [[ "${type}" -eq 1 ]]; then
  ...
  NAME="as-oft_coef1.0_kernal1.0_${TIMESTAMP}"
...
torchrun --standalone --nproc_per_node="${num_gpus}" ./core/train_ms3_ppo.py mani-skill-args \
  --name="${NAME}" \
  --env_id="${ENV_ID}" \
  --vla_path="${VLA_PATH}" \
