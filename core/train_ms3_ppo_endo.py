...
class Runner:
    ...
    def _get_endo_reward(self, obs, action):
        ...
        total_batch = obs["image"].shape[0]
        obs["image"] = obs["image"].to(self.ddp_device)
        ...
        for i in range(0, total_batch, self.args.buffer_inferbatch):
            ...
            reward = self.policy.get_endo_reward(obs_batch, actions_batch).detach()
    ...
    def render(self, epoch: int, env_type: str) -> dict:
        ...
        # 初始化汇总用的环境信息字典（三次循环都会累积到这里）
        total_env_infos = defaultdict(lambda: [])
        
        # 执行五次render循环
        eval_times = 2
        for cycle in range(eval_times):
            print(f"Starting render cycle {cycle + 1}/eval_times")
            ...
            episode_len = self.env.episode_len
            obs_img = to_tensor(obs_img)
            print(f"Cycle {cycle + 1} instruction[:3]:", instruction[:3])
            ...
            # 创建当前周期的保存目录
            exp_dir = Path(self.glob_dir) / f"vis_{epoch}_{env_type}_rank-{self.rank}_cycle-{cycle + 1}"
            ...
            for i in range(self.args.num_envs):
                ...
                assert len(images) == len(infos) + 1, f"Cycle {cycle + 1} env {i} image-info length mismatch"

                if self.args.render_info:
                    for j in range(len(infos)):
                        images[j + 1] = visualization.put_info_on_image(
                            images[j + 1], infos[j],
                            extras=[f"Ins: {instruction[i]}", f"Cycle: {cycle + 1}"]
                        )

                success = int(infos[-1]["success"])
                # 视频文件名包含周期编号和环境编号
                images_to_video(images, str(exp_dir), f"video_cycle-{cycle + 1}_env-{i}-s_{success}",
                                fps=10, verbose=False)

            print(f"Completed render cycle {cycle + 1}/eval_times\n")

        # 计算三次循环的汇总统计信息
        env_stats = {k: np.mean(v) for k, v in total_env_infos.items()}
        env_stats_ret = env_stats.copy()

        print(f"汇总{eval_times}次循环的统计信息:")
        ...
        # 构建汇总统计数据
        last_info = {
            idx: {k: total_env_infos[k][idx] for k in total_env_infos.keys() 
                if isinstance(total_env_infos[k], list) and len(total_env_infos[k]) > idx}
            for idx in range(self.args.num_envs * eval_times)  # 64*eval_times个环境信息
        }
        ...
        save_stats = {
            ...
            "epoch": epoch,
            "total_cycles": eval_times,
            "total_envs": self.args.num_envs * eval_times,
        ...
        if hasattr(args, 'only_eval') and args.only_eval:
            ...
            if args.benchmark == "libero":
                ...
                if args.env_id not in ll:
                    runner.render(epoch=0, env_type="train")
                if args.env_id != "PutOnPlateInScene25Main-v3":  # train env"
                    runner.render(epoch=0, env_type="test")
