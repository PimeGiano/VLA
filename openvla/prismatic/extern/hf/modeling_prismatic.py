...
class PrismaticForConditionalGeneration(PrismaticPreTrainedModel):
    ...
    def forward(
        ...
        # use_cache：生成任务，表示是否在生成时缓存中间状态（如 past_key_values），以加速自回归生成。
...
class OpenVLAForActionPredictionWithValueHead(PrismaticForConditionalGeneration):
    ...
    def evaluate_action(
        ...
        # 1. 输入验证：检查token序列格式
        # 2. 前向传播：获取隐藏状态和logits
        # 3. 价值计算：根据vh_mode选择不同的隐藏状态
        # 4. 概率计算：计算动作token的log概率
        # 5. 熵计算：计算动作分布的熵
        # 6. 返回：(logprobs, entropy, values)
        ...
        # 返回 (logprobs, entropy, values, logits_tensor)，以便上层训练可选使用 logits 进行分析/奖励计算
    ...
    def get_endo_reward(
        ...
        outputs = super().forward(
            ...
            pixel_values=pixel_values,
            labels=labels,
            output_hidden_states=False,  # output hidden_states -- do not need hidden states for endo reward
            return_dict=True,  # output dict
        )
    ...
    def predict_action_batch(
        ...
        # calculate endoreward
        # soft_logprobs_tensor = F.log_softmax(logits_tensor / alpha, dim=-1)
        # soft_logprobs = torch.gather(soft_logprobs_tensor, 2, idxes).squeeze(-1) # [B, action_len]
        # soft_logprobs = soft_logprobs.sum(dim=1, keepdim=True) # [B, 1]
        #
        # vq_logits_tensor = logits_tensor[:,0,:] # [B, 256]
        # vq_s1 = alpha * torch.log(torch.sum(torch.exp(vq_logits_tensor / alpha), dim = -1)).unsqueeze(-1)
        #
        # endo_reward = (soft_logprobs + vq_s1)/200

        return values, generated_ids, logprobs, logits_tensor
    
    
    def predict_endoreward_batch(
            self,
            input_ids: torch.LongTensor,
            attention_mask: torch.Tensor,
            pixel_values: torch.FloatTensor,
        ...
        generated_ids = output.sequences[:, -1: ] # [B, max_new_token]

        # check valid action
        # assert torch.all(generated_ids >= 32000 - 256) and torch.all(generated_ids < 32000)
        assert torch.all((generated_ids >= 32000 - 256) & (generated_ids < 32000))

        # logits
        logits_tensor = torch.stack(output.logits, dim=1) # [B, max_new_token, vocab_size + 64]
        logits_tensor = logits_tensor[:, 0:1, 32000 - 256 : 32000] # [B, 1, 256]
        logprobs_tensor = F.log_softmax(logits_tensor, dim=-1) # [B, 1, 256]

        idxes = actions.unsqueeze(-1) - (32000 - 256) # [B, 1, 1]
        endo_reward = torch.gather(logprobs_tensor, 2, idxes).squeeze(-1) # [B, 1]

        return endo_reward / 500
